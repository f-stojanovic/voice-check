...
describe('promotional-tone', () => {
  it('scores plain description 1.0', () => {
    expect(runRule(promotionalTone, 'Upit se vratio na osamdeset milisekundi.', 'sr').score).toBe(1);
  });
  ...
  it('matches Serbian adjectives across their inflections', () => {
    ...
    expect(runRule(promotionalTone, text, 'sr').findings.length).toBe(3);
  });
  ...
  it('reports the column of the first inflected match', () => {
    ...
    expect(positions(runRule(promotionalTone, text, 'sr'))).toEqual(['1:13']);
  });

  it('counts the English adjectives', () => {
    const text = 'An incredible, stunning, breathtaking result.';
    expect(runRule(promotionalTone, text, 'en').findings.length).toBe(3);
  });
});
