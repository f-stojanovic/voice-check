...
describe('participial-close', () => {
  it('scores prose that does not trail off 1.0', () => {
    expect(runRule(participialClose, 'The query came back to eighty milliseconds.', 'en').score).toBe(1);
  });
  ...
  it('counts exactly the closing participial clauses', () => {
    ...
    expect(runRule(participialClose, text, 'en').findings.length).toBe(2);
  });

  it('reports the position of the -ing word, not of the comma', () => {
    expect(positions(runRule(participialClose, 'The cache warms on boot, improving latency.', 'en'))).toEqual([
      '1:26',
    ]);
  });

  it('requires a comma, which removes some of the noise and not most of it', () => {
    expect(runRule(participialClose, 'The job is still running.', 'en').findings).toEqual([]);
  });
  ...
  it('fires on an adjective after a comma, which is the documented false positive', () => {
    ...
    const result = runRule(participialClose, 'The output was long, boring and repetitive.', 'en');
